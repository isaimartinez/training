            ...
            footerLinks.map((link) => (
              <div key={link.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                ...
                <ul className="list-none mt-4">
                  {
                    link.links.map((l,i) => (
                      <li key={l.name} className={`font-poppin font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${i !== link.links.length - 1 ? "mb-4" : "mb-9"}`}>
                        {l.name}
                      </li>
                    ))
                  }
